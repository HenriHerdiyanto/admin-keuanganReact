...
import LaporanProgramTable from "../../components/blud/LaporanProgramTable";
import LaporanDetailModal from "../../components/blud/LaporanDetailModal";

const API_BASE = "http://20.2.2.230/simkeu/public/api";

function LaporanBlud() {
  ...
  const fetchDataLaporan = useCallback(async (page = 1) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE}/lap_kegiatan_blud?page=${page}`,
        {
          method: "GET",
          headers: {
      ...
      if (Array.isArray(paginator)) {
        setDataProgram(paginator);
        setLastPage(1);
        setTotalData(paginator.length);
      } else {
      ...
      console.error("Gagal fetch laporan BLUD:", error);
      showToast(error.message || "Gagal mengambil data laporan", "error");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const openDetail = async (program) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${API_BASE}/lap_kegiatan_blud/detail/${program.idProgram}`,
        {
          method: "GET",
          headers: {
            Accept: "application/json",
      ...
      console.error("Gagal detail laporan BLUD:", error);
      showToast(error.message || "Gagal memuat detail laporan", "error");
    } finally {
