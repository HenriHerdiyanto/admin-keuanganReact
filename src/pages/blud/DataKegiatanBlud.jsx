...
function DataKegiatanBlud() {
  ...
  const fetchKegiatan = async (page = 1) => {
    try {
      ...
      const token = localStorage.getItem("token");
      const response = await fetch(
        `http://20.2.2.230/simkeu/public/api/data_kegiatan_blud?page=${page}`,
        {
          method: "GET",
          headers: {
            Accept: "application/json",
  ...
  const handleExportExcel = () => {
    exportToExcel({
      ...
      sheetName: "Kegiatan BLUD",
      fileName: `Data_Kegiatan_BLUD_${new Date().toISOString().split("T")[0]}`,
    });
  };
  ...
  function handleDelete(id, namaKegiatan) {
    confirmDelete({
      url: "http://20.2.2.230/simkeu/public/api/data_kegiatan_blud",
      id,
...
export default DataKegiatanBlud;
