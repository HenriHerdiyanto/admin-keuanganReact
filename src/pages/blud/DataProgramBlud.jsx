...
import FormProgramBlud from "../../components/blud/FormProgramBlud";
...
function DataProgramBlud() {
  ...
  const fetchDataProgram = async (page = 1) => {
    try {
      ...
      const response = await fetch(
        `http://20.2.2.230/simkeu/public/api/data_blud?page=${page}`,
        {
          method: "GET",
          headers: {
  ...
  const handleExportExcel = () => {
    exportToExcel({
      ...
      sheetName: "Program BLUD",
      fileName: `Data_Program_BLUD_${new Date().toISOString().split("T")[0]}`,
    });
  };
  ...
  function handleDelete(id) {
    confirmDelete({
      url: "http://20.2.2.230/simkeu/public/api/data_blud",
      id,
      onSuccess: () => {
...
export default DataProgramBlud;
